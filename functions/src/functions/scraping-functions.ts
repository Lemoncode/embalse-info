...
export async function scrapingsFunction(
  ...
  try {
    ...
    const responseCuencaCantabrico =
      await embalsesRepository.actualizarCuencaCantabrico();
    ...
    if (responseCuencaCantabrico) {
      context.log(
        "scrapings-function: Se han actualizado los embalses de la cuenca del Cantábrico",
      );
    } else {
      context.log(
        "scrapings-function: No se han podido actualizar los embalses de la cuenca del Cantábrico",
      );
    }
  } catch (error) {
