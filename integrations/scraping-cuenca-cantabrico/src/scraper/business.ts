...
const get = <T = unknown>(o: Record<string, unknown> | undefined, k: string): T | undefined =>
  (o?.[k] as T | undefined);
...
export function toRawRows(features: SaichFeature[]): RawRow[] {
  return features
    .filter(f => {
      ...
      const tipo = (get<string>(p, "tipo_estacion") ?? get<string>(p, "tipo_senial") ?? "").toLowerCase();
      return tipo === "embalse";
    })
    .map((f) => {
      const p = f.properties as Record<string, unknown> | undefined;

      const idStr = (get<string>(p, "codigo_general") ?? get<string>(p, "Cod_Roea") ?? "0").toString();
